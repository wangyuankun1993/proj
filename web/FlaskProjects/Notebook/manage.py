...
# File Name: manage
# Date: 2022/01/12 09:56:25
# Author: wangyuankun
# Function: This file is to achieve manage!
...
def add_article():
    ...
    if request.method == 'POST' and form.validate(): # 如果用户提交表单，并且表单验证通过
        ...
        db = MysqlUtil() # 实例化数据库操作类
        sql = "INSERT INTO articles(title,content,,author,create_date) VALUES ('%s','%s','%s','%s')" % (title,content,author,create_date) # 插入数据的SQL语句
        db.insert(sql)
